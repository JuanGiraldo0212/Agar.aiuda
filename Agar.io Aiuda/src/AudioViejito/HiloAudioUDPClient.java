...
public class HiloAudioUDPClient extends Thread{
	...
	public HiloAudioUDPClient(Client client) {
		
		this.client = client;
		soundFile = new File(client.MUSIC_PATH);
	}
	
	public void run() {
		
		try {
			client.setSocketMusic(new MulticastSocket(Client.MUSIC_PORT));
			InetAddress inetAddress = InetAddress.getByName(client.MULTICAST_IP);
			client.getSocketMusic().joinGroup(inetAddress);
			initiateAudio();
		} catch (IOException e) {
	...
	private void initiateAudio() {
		try {
			MulticastSocket socket = client.getSocketMusic();
